...
export function SettingsBar() {
  ...
  const handlePhotoUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_PHOTO_SIZE) {
      console.error('[SettingsBar] File size exceeds 1GB limit:', file.size);
      alert(`File size exceeds 1GB limit. Your file is ${(file.size / (1024 * 1024)).toFixed(1)}MB. Please select a smaller image.`);
      return;
    }
